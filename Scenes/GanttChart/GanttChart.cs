...
public class GanttChart : Panel
{
    ...
    public override void _Ready()
    {
        ...
        _ganttContent = GetNode<Control>("ScrollContainer/GanttContent");

        // Update download button text
        UpdateDownloadButtonText();
    }

    public void UpdateGanttChart(List<Traveller.TravellerInfo> currentTravellerData, float currentTime)
    {
    ...
    public void ResetGanttChart()
    {
        ...
        GD.Print("Gantt chart reset - cleared persistent timelines and chart content");
    }

    private void UpdateDownloadButtonText()
    {
        var downloadButton = GetNodeOrNull<Button>("DownloadButton");
    ...
    private string GetActivityShortName(Traveller.TravellerActivity activity)
    {
        switch (activity)
        {
            ...
            case Traveller.TravellerActivity.PickingUp: return "Pickup";
            case Traveller.TravellerActivity.MovingToDelivery: return "→Delivery";
            case Traveller.TravellerActivity.Delivering: return "Deliver";
    ...
    private void OnDownloadCsvPressed()
    {
        ...
        string fileName = $"process_simulation_timeline_{DateTime.Now:yyyyMMdd_HHmmss}.csv";

        // For web exports, use JavaScript to download
        ...
        {
            // For desktop, save to user data directory
            SaveCsvToFile(csvContent, fileName);
        }

        GD.Print($"CSV export completed: {fileName}");
    }
    ...
    private void SaveCsvToFile(string csvContent, string fileName)
    {
        ...
        if (file.Open(filePath, File.ModeFlags.Write) == Error.Ok)
        {
            ...
            file.Close();
            GD.Print($"CSV saved to: {OS.GetUserDataDir()}/{fileName}");
        }
        else
        {
            GD.PrintErr($"Failed to save CSV file: {fileName}");
        }
    }
    ...
    private void DownloadCsvOnWeb(string csvContent, string fileName)
    {
        ...
        string encodedContent = System.Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(csvContent));
        string dataUrl = $"data:text/csv;base64,{encodedContent}";
    ...
    // Public method to get timeline count for external access
    public int GetTimelineCount()
    {
        return _persistentTimelines.Count;
    }

    // Public method to get persistent timelines for external access
    public Dictionary<int, Traveller.TravellerInfo> GetPersistentTimelines()
    {
        return new Dictionary<int, Traveller.TravellerInfo>(_persistentTimelines);
    }
}
